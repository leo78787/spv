 ...
 * account with this email. Returns null for unknown emails and for the legacy
 * username login (no real address). Doesn't touch the current password.
 */
export function createPasswordResetByEmail(email: string): { user: AdminUser; oneTimePassword: string } | null {
  const data = load();
  const normalizedEmail = email.trim().toLowerCase();
